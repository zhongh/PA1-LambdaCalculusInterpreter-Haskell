...
runProgram fileName reducer = do
    fcontents <- readFile fileName   -- fcontents: file contents
    let inList = lines fcontents     -- inList: file contents in list of lines
    let parsedList = parseLEList inList   -- parsedList: parsed version of inList using "parseLEList"
    --putStrLn ("The type of parsedList is" ++ show (parsedList))
    let reducedList = map reducer parsedList  -- reducedList: reduced list using "reducer"
    --let reducedListAlpha = map reducerAlpha reducedList   -- alpha renaming
    --let reducedListBeta = map reducerBeta reducedListAlpha  -- beta reduction
    --let reducedListEta = map reducerEta reducedListBeta     -- eta conversion

    outputPrinter 1 (zip parsedList reducedList)   -- displaying results
